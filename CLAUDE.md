...
- โลโก้เก็บใน R2 เหมือนสลิป แต่เสิร์ฟผ่าน **`GET /api/branding` ที่ไม่ต้องล็อกอิน**
  คืน `{ company_name, logo_url }` โดย `logo_url` เป็น presigned GET · ใส่ `Cache-Control` ได้เพราะไม่ใช่ความลับ
