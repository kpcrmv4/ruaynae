...
try {
  ...
  // ── P2-CALC-08 · หัวหน้าไซต์ไม่เห็นตัวเลขเงินของไซต์เลย ─────────────
  ...
  // แม้หน้าจะว่างเปล่าเพราะเหตุอื่น (เช่นโดน 404)
  {
    ...
    const leaks = ['เก็บเงินแล้ว', 'กำไรคงเหลือ', 'ค่างานตามสัญญา', 'ต้นทุนที่จ่ายจริง',
      'ต้นทุนไซต์นี้', '1,000,000', '400,000', '300,000']
      .filter((w) => html.includes(w))
    // ฝั่งบวก: ยังเปิดหน้าไซต์ตัวเองได้ และยังเห็นรายจ่ายที่ตัวเองคีย์ที่ /ledger
    const opensPage = html.includes(NAME.a) && html.includes('ความคืบหน้า')
    const ledger = await page('/ledger', supJar)
    check('P2-CALC-08 หัวหน้าไซต์ไม่เห็นตัวเลขเงินบนหน้าไซต์เลย · แต่ยังเปิดหน้าได้และเห็นรายการที่ตัวเองคีย์',
      leaks.length === 0 && opensPage && ledger.includes('ต้นทุนทดสอบ A'),
      leaks.length ? `หลุด: ${leaks.join(', ')}` : 'ไม่มีเงินบนหน้าไซต์ · เห็นรายการใน /ledger')
  }
