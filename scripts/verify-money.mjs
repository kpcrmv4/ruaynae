 ...
 *
 * 🔴 อ่านตัวเลขจาก **การ์ดของโครงการนั้น** ไม่ใช่จากทั้งหน้า
...
/** ชิ้นส่วน HTML ของการ์ดโครงการใบเดียว — ตัวเลขที่อ่านได้จึงเป็นของโครงการนั้นแน่ ๆ */
...
const NAME = { a: 'ทดสอบเงิน โครงการกำไรดี', b: 'ทดสอบเงิน โครงการต้นทุนแซง', c: 'ทดสอบเงิน โครงการยังไม่ตั้งค่างาน' }
let A = null
let B = null
let C = null
...
try {
  ...
  // โครงการทดสอบจึงอยู่ต้นลิสต์เสมอ ไม่ว่าฐานข้อมูลจะมีโครงการจริงกี่โครงการ
  ...
  // ป้ายที่ขึ้นทุกใบก็ไม่ได้บอกอะไร และตรวจแค่ใบเดียวจะแยกสองกรณีนี้ไม่ออก
  {
    ...
    check('P2-CALC-03 P1-CALC-07 โครงการที่ต้นทุน% แซงเก็บเงิน% ขึ้นป้าย · โครงการที่ยังไม่แซงไม่ขึ้น (หน้าเดียวกัน)',
  ...
  // ── P2-CALC-07 · ค่างาน 0 → ห้ามหารด้วยศูนย์ ────────────────────────
  {
    ...
    check('P2-CALC-07 โครงการที่ยังไม่ได้ตั้งค่างาน → บอกตรง ๆ ไม่มี NaN/Infinity',
      html.includes('ยังไม่ได้ตั้ง') && bad.length === 0,
      bad.length ? `เจอ ${bad.join(', ')}` : 'ข้อความแทนเปอร์เซ็นต์')
  }

  // ── P2-CALC-08 · หัวหน้าโครงการไม่เห็นตัวเลขเงินของโครงการเลย ─────────────
  // เจ้าของสั่งไว้ 31 ส.ค. 2569: เขามีหน้าที่แค่บันทึกรายจ่ายและบันทึกว่า
  // วันนี้ใครมาทำงาน · เดิมแถวนี้ยอมให้เห็น "ยอดรายจ่ายที่อนุมัติแล้ว" ได้
  //
  ...
  // แม้หน้าจะว่างเปล่าเพราะเหตุอื่น (เช่นโดน 404)
  {
    ...
    const opensPage = html.includes(NAME.a) && html.includes('ความคืบหน้า')
    const ledger = await page('/ledger', supJar)
    check('P2-CALC-08 หัวหน้าโครงการไม่เห็นตัวเลขเงินบนหน้าโครงการเลย · แต่ยังเปิดหน้าได้และเห็นรายการที่ตัวเองคีย์',
      leaks.length === 0 && opensPage && ledger.includes('ต้นทุนทดสอบ A'),
      leaks.length ? `หลุด: ${leaks.join(', ')}` : 'ไม่มีเงินบนหน้าโครงการ · เห็นรายการใน /ledger')
  }
