...
22. **สคริปต์ลบข้อมูลที่ "ก๊อปชื่อมาจาก seed" คือเขียวปลอมรอวันเกิด**
   `remove-demo-data.mjs` ถือรายชื่อโครงการชุดกรุงเทพ ('บ้านคุณสมศักดิ์ ซ.รามอินทรา 65' ฯลฯ)
   ส่วน `seed-demo.mjs` ย้ายไปใช้ชื่อลำปางแล้ว ('บ้านคุณสมศักดิ์ ต.บ่อแฮ้ว' ฯลฯ) · สองไฟล์นี้
   ไม่มีอะไรผูกกันเลยนอกจากความตั้งใจของคนแก้ · ผลคือสคริปต์ `--confirm` ลบได้ **0 แถว**
   แล้วพิมพ์ *"ไม่เหลือแถวของชุดเดโม่แล้ว"* พร้อม **exit 0** — คำตอบที่ถูกต้องตามตรรกะของมันเอง
   และผิดสนิทกับความจริง · ไม่มี error ที่ไหน ไม่มีอะไรแดง คนสั่งเชื่อว่าฐานสะอาดแล้ว
   แล้วเปิดใช้จริงทับข้อมูลเดโม่
   · เจอตอนเจ้าของสั่งเคลียร์ seed ก่อนใช้จริง (20 ก.ย. 2569) — ต้องลบด้วย SQL ตรงแทน
   · **แก้:** สคริปต์อ่านซอร์สของ `seed-demo.mjs` แล้ว assert ว่าทุกชื่อที่มันจะลบยังมีอยู่จริง
   ในนั้น ไม่งั้น throw · หลักเดียวกับข้อ 11 — **สำเนาที่ไม่มีตัวตรวจว่าตรงต้นฉบับ
   ไม่ใช่สำเนา มันคือไฟล์คนละใบที่บังเอิญเคยเหมือนกัน**
