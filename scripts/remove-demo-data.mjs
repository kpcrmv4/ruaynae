 ...
 * insert ให้หัวหน้าโครงการคนเดียวกัน → มอบหมายไม่สำเร็จ → หัวหน้าโครงการไม่มีสิทธิ์
 * ในโครงการทดสอบ → **ทุกแถวที่ต้องใช้สิทธิ์หัวหน้าโครงการตกยกชุด (30 แถว)**
 ...
 * · รายการส่วนกลาง (`site_id is null`) ไม่มีโครงการให้ยึด จึงยึดจาก `note`
...
const SITES = [
  'บ้านคุณสมศักดิ์ ต.บ่อแฮ้ว',
  'โกดังโรงงานเซรามิกลำปาง',
  'รีโนเวทอาคารพาณิชย์ 2 คูหา กาดกองต้า',
  'ต่อเติมครัวหลังบ้าน ต.พระบาท',
  'บ้านแฝดสองชั้น อ.เกาะคา (รอเซ็นสัญญา)',
]
const EMPLOYEES = [
  'สมพงษ์ ใจดี', 'บุญมี แซ่ลิ้ม', 'วิชัย ทองสุข',
  'สมหมาย เพ็ชรดี', 'ประเสริฐ มั่นคง', 'จำลอง หายไป',
]
/** รายจ่ายส่วนกลางของชุดเดโม่ — ไม่มีโครงการให้ยึด จึงยึดจาก note */
const CENTRAL_NOTES = ['ค่าน้ำมันรถกระบะเดือนนี้', 'ค่าเช่าออฟฟิศและอินเทอร์เน็ต']

/**
 * 🔴 รายชื่อข้างบนเป็นสำเนาของ `seed-demo.mjs` — ถ้า seed เปลี่ยนชื่อแล้วที่นี่ไม่เปลี่ยน
 * สคริปต์จะลบได้ 0 แถวแล้วรายงานว่า "ไม่เหลือแถวของชุดเดโม่แล้ว" ซึ่งเป็นเขียวปลอม
 * (เกิดขึ้นมาแล้วจริง: seed ย้ายจากชื่อกรุงเทพเป็นชื่อลำปาง ที่นี่ค้างชุดเก่าไว้)
 * · เทียบกับซอร์สของ seed ทุกครั้งที่รัน ดีกว่าเชื่อว่าคนจะแก้สองที่พร้อมกัน
 */
const seedSrc = readFileSync('scripts/seed-demo.mjs', 'utf8')
const drifted = [...SITES, ...EMPLOYEES, ...CENTRAL_NOTES].filter((n) => !seedSrc.includes(n))
if (drifted.length) {
  throw new Error(
    `ชื่อในสคริปต์นี้ไม่มีอยู่ใน seed-demo.mjs แล้ว — ลบไปก็ไม่โดนอะไร:\n  ` +
    drifted.join('\n  ') +
    '\nแก้รายชื่อให้ตรงกับ seed ก่อนรันใหม่',
  )
}
...
console.log('\nโครงการที่ **เก็บไว้**:', keep.length ? keep.map((r) => r.name).join(' · ') : '(ไม่มี)')
